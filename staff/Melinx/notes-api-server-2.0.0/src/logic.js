...
const logic = {
    ...
    addNote(userId, text) {
        ...
        if (typeof userId !== 'string') throw Error('text is not a string')
    ...
    retrieveNote(userdId, id) {
        ...
        if (note.userId !== note.id) throw Error(`Note with ID ${note.id} does not match given UserId`)
        ...
        const index = this._notes.findIndex(note => {if(note.id === id && note.userId === userId) return note})
    ...
    listNotes(userId) {
        ...
        return this._notes.filter(v => { if (v.userId) return v })
    },
    ...
    updateNote(userId, id, text) {
        ...
        const note = this._notes.find(note => {if(note.id === id && note.userId === userId) return note})
