...
const logic = {
    ...
    findNotes(str){
        if (typeof str !== 'string') throw Error('text is not a string')
        if(!str.length) throw Error('text is not a string')

        let noteSearch =  this._notes.filter(v => (v.text.toLowerCase()).includes(str).toLowerCase())
        return noteSearch
    }
}
